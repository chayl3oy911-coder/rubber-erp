...
export default async function StockListPage({
  ...
  const parsed = listStockLotsQuerySchema.safeParse({
    ...
    rubberType: pickString(sp, "rubberType"),
  ...
  const result = await listStockLots(me, {
    ...query,
    status: query.status as ReadonlyArray<StockLotStatus> | undefined,
  });

  const canCreate = hasPermission(me, "stock.create");
    ...
    <div className="flex flex-col gap-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canCreate ? (
            <Link href="/stock/from-purchase" className={primaryButtonClass}>
              {t.actions.fromPurchase}
            </Link>
          ) : null}
        </div>
      </header>

      <StockSearch initialQ={query.q ?? ""} />
      ...
      <StockFilters
        ...
        selectedRubberType={query.rubberType}
